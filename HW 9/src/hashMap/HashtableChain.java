...
public class HashtableChain<K, V> extends AbstractMap<K, V> implements KWHashMap<K, V> {
	...
	/**
	 * Inner class to implement the set iterator. It contians a next, hasNext, and remove.
	 * */
	private class SetIterator implements Iterator<Map.Entry<K, V>> {
		...
		/**
		 * Returns the next value. Throws NoSuchElementException if there is no next element.
	...
	/**
	 * The helper method rehash doubles the table size + 1 when the table has past its load capacity.
	 */
	private void rehash() {
